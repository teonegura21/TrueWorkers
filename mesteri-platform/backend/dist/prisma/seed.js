...
async function seedUsers() {
    ...
    const passwordHash = await bcrypt_1.default.hash('Secret123!', 10);
    ...
    craftsmen.push(await prisma.user.create({
        data: {
            ...
            userType: client_1.UserType.SRL,
    ...
    craftsmen.push(await prisma.user.create({
        data: {
            ...
            userType: client_1.UserType.PFA,
            role: client_1.UserRole.CRAFTSMAN,
            isVerified: true,
            averageRating: 4.8,
            totalReviews: 98,
...
async function seedProjectData(clientId, craftsmanId) {
    ...
    const conversation = await prisma.conversation.create({
        data: {
            projectId: project.id,
            type: client_1.ConversationType.PROJECT,
        ...
        data: [
            {
                ...
                userId: clientId,
                role: client_1.ConversationParticipantRole.CLIENT,
            },
            {
                conversationId: conversation.id,
                userId: craftsmanId,
                role: client_1.ConversationParticipantRole.MESTER,
            },
        ],
    });
    ...
    const attachment = await prisma.attachment.create({
        data: {
            ...
            retentionPolicyId: supportPolicy.id,
            status: client_1.AttachmentStatus.ACTIVE,
        },
    });
    ...
    await prisma.projectEvent.create({
        data: {
            projectId: project.id,
            actorId: craftsmanId,
            eventType: client_1.ProjectEventType.STATUS_CHANGE,
            payload: {
                from: 'PLANNED',
                to: 'IN_PROGRESS',
...
async function seedInspirationPosts(craftsmen) {
...
async function seedReviews(client, craftsmen) {
