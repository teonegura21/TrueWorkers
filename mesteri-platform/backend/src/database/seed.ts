import { PrismaClient, UserRole, UserType, ProjectStatus, JobStatus, JobCategory, UrgencyLevel } from '@prisma/client';

const prisma = new PrismaClient();

/**
 * Database Seeder - Populate with realistic Romanian test data
...
async function main() {
  ...
  for (const craftsmanData of romanianNames.craftsmen) {
    const craftsman = await prisma.user.create({
      data: {
        ...
        isVerified: true, // All craftsmen are verified
  ...
  // Create jobs first (which will then have projects)
  console.log('📋 Creating jobs and projects...');
  const jobStatusOptions = [JobStatus.ACTIVE, JobStatus.ACCEPTED, JobStatus.IN_PROGRESS, JobStatus.COMPLETED];
  const projectStatusOptions = [ProjectStatus.FUNDED, ProjectStatus.ACTIVE, ProjectStatus.COMPLETED, ProjectStatus.CANCELLED];
  ...
  for (let i = 0; i < 50; i++) {
    ...
    // Map category to JobCategory enum
    let category: JobCategory = JobCategory.ALTELE;
    if (template.category.includes('electrician')) category = JobCategory.ELECTRIK;
    else if (template.category.includes('instalator')) category = JobCategory.INSTALATII_SANITARE;
    else if (template.category.includes('constructii')) category = JobCategory.CONSTRUCTII;
    ...
    const job = await prisma.job.create({
      data: {
        ...
        budgetMin: 200,
        budgetMax: 1000,
        urgency: Math.random() > 0.7 ? UrgencyLevel.HIGH : Math.random() > 0.4 ? UrgencyLevel.MEDIUM : UrgencyLevel.LOW,
        status: jobStatus,
        clientId: client.id,
      },
    });

    // Create offers for jobs
    if (jobStatus === JobStatus.ACTIVE && Math.random() > 0.5) {
      const numOffers = Math.floor(Math.random() * 3) + 1; // 1-3 offers
      for (let j = 0; j < numOffers; j++) {
        ...
        await prisma.offer.create({
          data: {
            ...
            bidAmount: Math.floor(Math.random() * 800) + 200, // 200-1000 RON
            estimatedDays: Math.floor(Math.random() * 10) + 1, // 1-10 days
    ...
    // Create project if job is accepted/in progress/completed
    if (jobStatus !== JobStatus.ACTIVE) {
      ...
      await prisma.project.create({
        data: {
          ...
          startDate: projectStatus !== ProjectStatus.FUNDED 
            ? new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000) 
            : null,
          endDate: projectStatus === ProjectStatus.COMPLETED 
